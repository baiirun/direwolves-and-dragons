...
const PartyCard = ({ party }: Props) => {
  const characters = party.characters.map((character: Character) => (
    <CharacterAvatar
      alt={`Character avatar of ${character.name}`}
      key={character.id}
...
const Card = styled(Link)`
  border-radius: 4px;
  box-shadow: 0 10px 20px hsla(0, 0%, 27%, 0.08),
    0 3px 6px hsla(0, 0%, 23%, 0.1);
  margin-right: 27px;
  margin-bottom: 54px;
  width: 250px;
  height: 300px;
  ...
  &:hover {
    ...
    box-shadow: 0 16px 32px hsla(0, 0%, 0%, 0.1),
      0 3px 12px hsla(0, 0%, 15%, 0.05);
  }
